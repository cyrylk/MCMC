...
def get_max_monogram_state_coord(text, coordinate, monogram_dist, key_length, alphabet):
    max_state = 0
    max_func = 0
    for j in vigenere.get_all_mono_keys(alphabet):
        func = 0
...
def get_max_bigram_state(text, bigram_dist, key_length, alphabet):
    all_single_keys = vigenere.get_all_mono_keys(alphabet)
    codes = {a: {b: [vigenere.get_zero_mono_key() for i in range(key_length - 1)] for b in all_single_keys} for a in all_single_keys}
